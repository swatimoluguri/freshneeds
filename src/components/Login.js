import Food from "../assets/food.jpg";
import { useState, useContext } from "react";
import UserContext from "../utils/UserContext";

const Login = () => {
  const [password, setPassword] = useState({
    value: "",
    isTouched: false,
  });
  const [username, setUsername] = useState("");
  const {setLoggedUser} = useContext(UserContext);
  const getIsFormValid = () => {
    return username && password.value.length >= 8;
  };
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoggedUser(username);
    alert("Account Created! Welcome onboard " + username);
  };
  return (
    <>
      <div className="relative inline-block">
        <img src={Food} alt="food-background" className="w-full h-auto block" />
        <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-50">
          <div className="relative w-1/3 bg-black bg-opacity-60 mx-auto mt-36 left-0 right-0 align-middle ">
            <div className="flex flex-col p-8 my-4">
              <h3 className="font-bold text-2xl my-2 p-2 text-white">
                Sign In
              </h3>
              <form className="flex flex-col" onSubmit={handleSubmit}>
                <input
                  className="my-4 p-4 bg-transparent border-white border rounded-lg text-white"
                  type="text"
                  placeholder="Username"
                  onChange={(e) => {
                    setUsername(e.target.value);
                  }}
                />

                <input
                  className="my-4 p-4 bg-transparent border-white border rounded-lg text-white"
                  type="password"
                  placeholder="Password"
                  onChange={(e) => {
                    setPassword({ ...password, value: e.target.value });
                  }}
                  onBlur={() => {
                    setPassword({ ...password, isTouched: true });
                  }}
                />
                {password.isTouched && password.value.length < 8 ? (
                  <p className="text-red-500">
                    Password should have at least 8 characters
                  </p>
                ) : null}
                <button
                  className="bg-green-500 my-4 p-4 rounded-lg font-semibold"
                  disabled={!getIsFormValid()}
                >
                  Sign In
                </button>
              </form>

              <h3 className="text-white">
                New to freshneeds?{" "}
                <span className="font-bold hover:text-green-500 hover:cursor-pointer">
                  Sign Up now
                </span>{" "}
              </h3>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};
export default Login;
